...
## Remote (SaaS) — fastest
1. In Penpot: **Your Account → Integrations → MCP Server**, enable it.
2. Generate a personal **MCP Key** and store it in a password manager.
3. The server URL is `https://design.penpot.app/mcp/stream?userToken=YOUR_SECRET_MCP_KEY`.
4. Use `claude-desktop-config.remote.json` / `cursor-mcp-config.json`.
